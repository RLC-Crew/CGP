...
namespace CGPWinWebLogica.Utilidades
{
    public static class Utilidades
    {
        /// <summary>
        /// Genera una contraseña basado en un Guid, por lo que no contiene caracteres especiales.
        /// </summary>
        /// <param name="largo">El largo max(32)</param>
        /// <returns></returns>
        public static string GenerarContrasena(int largo)
        {
            // Obtener id.
            ...
            // Verificar largo
            if (largo <= 0 || largo > guidResult.Length)
                throw new ArgumentException("El largo debe estar entre 1 y " + guidResult.Length);

            // Retornar el substring.
            return guidResult.Substring(0, largo);

        }

        /// <summary>
        /// Genera una contrasena alfanumerica con carcteres especiales.
        /// </summary>
        /// <param name="minLength">largo mínimo.</param>
        /// <param name="maxLength">largo máximo.</param>
        /// <returns></returns>
        public static string GenerarContrasenaEspecial(int minLength, int maxLength)
        {
            // Define default min and max password lengths.
            //int DEFAULT_MIN_PASSWORD_LENGTH  = 8;
            //int DEFAULT_MAX_PASSWORD_LENGTH  = 10;
        ...
        public static void Sort<T>(this List<T> list, string sortExpression)
        {
            ...
            foreach (string sortExpress in sortExpressions)
            {
                ...
                DireccionOrdenamiento SortDirection = DireccionOrdenamiento.Ascendente;
                if (sortDirection.ToLower().Equals("asc") || sortDirection.ToLower().Equals("ascending"))
                {
                    SortDirection = DireccionOrdenamiento.Ascendente;
                }
                else if (sortDirection.ToLower().Equals("desc") || sortDirection.ToLower().Equals("descending"))
                {
                    SortDirection = DireccionOrdenamiento.Descendente;
                }
                else
                {
        ...
        private class GenericComparer
        {
            public List<GenericComparer> comparers { get; set; }
            int level = 0;

            public DireccionOrdenamiento SortDirection { get; set; }
            ...
            public int Compare<T>(T t1, T t2)
            {
                ...
                {
                    if (comparers[level].SortDirection == DireccionOrdenamiento.Descendente)
                    {
                        ret *= -1;
                    }
                }
